            ...
            |span, identifier_container_path, identifier_name| {
                ...
                match top {
                    Node::Config(config) => if let Some(identifier) = identifier_name {
                        let item = config.dictionary_literal().expressions().find(|e| e.key().named_key_without_resolving() == Some(identifier)).unwrap();
                        vec![Definition {
                            path: schema.source(config.source_id()).unwrap().file_path.clone(),
                            selection_span: span,
                            target_span: item.key().span(),
                            identifier_span: item.key().span(),
                        }]
                    } else {
                        vec![Definition {
                            path: schema.source(config.source_id()).unwrap().file_path.clone(),
