...
fn jump_to_definition_in_type_item(
    ...
    if type_item.identifier_path.span.contains_line_col(line_col) {
        ...
        let reference = search_identifier_path_names_with_filter_to_type_and_value(schema, source, namespace_path, &user_typed_spaces, &top_filter_for_reference_type(ReferenceType::Default), availability);
