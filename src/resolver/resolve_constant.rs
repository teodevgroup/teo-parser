...
pub(super) fn resolve_constant<'a>(constant: &Constant, context: &'a ResolverContext<'a>) {
    ...
    constant.resolve(ConstantResolved {
        accessible: resolve_expression_kind(&constant.expression, context, constant.type_expr.map_or(&undetermined, |t| t.resolved())),
    });
}