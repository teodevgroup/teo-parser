...
pub(super) fn find_completion_in_dictionary_literal(schema: &Schema, source: &Source, dictionary_literal: &DictionaryLiteral, line_col: (usize, usize), namespace_path: &Vec<&str>) -> Vec<CompletionItem> {
    for (key_expression, value_expression) in &dictionary_literal.expressions {
        if key_expression.span().contains_line_col(line_col) {
            return find_completion_in_expression(schema, source, key_expression, line_col, namespace_path);
        }
        if value_expression.span().contains_line_col(line_col) {
            return find_completion_in_expression(schema, source, value_expression, line_col, namespace_path);
        }
    }
    vec![]
}
