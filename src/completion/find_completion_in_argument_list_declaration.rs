...
use crate::completion::find_completion_in_type_expr::{find_completion_for_empty_type_item, find_completion_in_type_expr, TypeExprFilter};
...
pub(super) fn find_completion_in_argument_list_declaration(schema: &Schema, source: &Source, argument_list_declaration: &ArgumentListDeclaration, line_col: (usize, usize), generics: &Vec<&GenericsDeclaration>, namespace_path: &Vec<&str>, availability: Availability) -> Vec<CompletionItem> {
    for partial_argument_declaration in argument_list_declaration.partial_argument_declarations() {
        if partial_argument_declaration.span.contains_line_col(line_col) && !partial_argument_declaration.identifier().span.contains_line_col(line_col) {
            return find_completion_for_empty_type_item(schema, source, namespace_path, TypeExprFilter::None, availability);
        }
    }
