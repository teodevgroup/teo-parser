    ...
    if function_declaration.span.contains_line_col(line_col) {
        ...
        if let Some(argument_list_declaration) = &function_declaration.argument_list_declaration {
            if argument_list_declaration.span.contains_line_col(line_col) {
                return find_completion_in_argument_list_declaration(schema, source, argument_list_declaration, line_col, &generics, &function_declaration.namespace_str_path(), function_declaration.define_availability);
            }
        }
