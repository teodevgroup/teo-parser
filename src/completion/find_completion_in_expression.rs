...
pub(super) fn find_completion_in_expression_kind(schema: &Schema, source: &Source, kind: &ExpressionKind, line_col: (usize, usize), namespace_path: &Vec<&str>, expect: &Type, availability: Availability) -> Vec<CompletionItem> {
    match kind {
        ExpressionKind::Group(g) => find_completion_in_expression(schema, source, g.expression(), line_col, namespace_path, expect, availability),
        ExpressionKind::ArithExpr(arith) => find_completion_in_arith_expr(schema, source, arith, line_col, namespace_path, expect, availability),
        ...
        ExpressionKind::TupleLiteral(tuple) => find_completion_in_tuple_literal(schema, source, tuple, line_col, namespace_path, expect, availability),
        ExpressionKind::ArrayLiteral(array) => find_completion_in_array_literal(schema, source, array, line_col, namespace_path, expect, availability),
        ExpressionKind::DictionaryLiteral(dictionary) => find_completion_in_dictionary_literal(schema, source, dictionary, line_col, namespace_path, expect, availability),
        ...
        ExpressionKind::Unit(unit) => find_completion_in_unit(schema, source, unit, line_col, namespace_path, expect, availability),
