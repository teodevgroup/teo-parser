    ...
    for expression in unit.expressions.iter() {
        if let Some(identifier) = expression.kind.as_identifier() {
            if identifier.span.contains_line_col(line_col) {
                ...
                if let Some(reference) = search_identifier_path_names_with_filter(schema, source, namespace_path, &user_typed_prefix, &top_filter_for_pipeline(), availability) {
            ...
            if argument_list.span.contains_line_col(line_col) {
                return handle_argument_list(argument_list, search_identifier_path_names_with_filter(schema, source, namespace_path, &user_typed_prefix, &top_filter_for_pipeline(), availability).map(|r| schema.find_top_by_path(&r).unwrap().as_pipeline_item_declaration()).flatten());
            } else {
                user_typed_prefix = vec![];
            }
        }
